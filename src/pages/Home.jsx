...
import { getCurrentFarmer, setCurrentFarmer } from "../lib/currentFarmer"
...
function SetupForm({ onDone }) {
  ...
  const next = () => {
    ...
    if (name.trim()) {
      const farmer = { id: crypto.randomUUID(), name: name.trim(), village: village.trim(), crop: crop.trim(), location, addedAt: new Date().toISOString() }
  ...
  const questions = [t("whatYourName"), t("whichVillage"), t("whatGrow")]
  const placeholders = [t("namePlaceholder"), t("villagePlaceholder"), t("cropPlaceholder")]

  return (
    <div className="min-h-screen bg-gradient-to-b from-primary/5 to-base-100 flex flex-col items-center justify-center px-4 py-10">
      <SunLogo className="w-20 h-20 mb-4" />
      <h1 className="font-display text-3xl font-extrabold text-neutral mb-2">{t("appName")}</h1>
      <p className="text-base-content/70 mb-6">{t("tagline")}</p>
      ...
      <div className="bg-base-100 rounded-box shadow-xl p-8 w-full max-w-lg">
        <div className="text-center mb-6">
          <p className="text-sm text-base-content/50 mb-2">{t("step")} {step + 1} {t("ofSteps")} 3</p>
          <AudioText text={questions[step]} as="h2" className="font-display text-2xl font-bold text-neutral justify-center" />
          <p className="text-sm text-base-content/60 mt-2">{t("setupDesc")}</p>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            <input
              ...
              value={val}
              onChange={(e) => { if (step === 0) setName(e.target.value); else if (step === 1) setVillage(e.target.value); else setCrop(e.target.value) }}
              onKeyDown={(e) => e.key === "Enter" && val.trim() && next()}
              placeholder={placeholders[step]}
              autoFocus
            />
            <VoiceInput onResult={(text) => { if (step === 0) setName(text); else if (step === 1) setVillage(text); else setCrop(text) }} />
          </div>

          {step === 2 && (
            <button type="button" className="btn btn-outline btn-sm gap-2" onClick={fetchLocation} disabled={locating}>
              {locating ? t("finding") : t("useGps")}
            </button>
          )}

          <button className="btn btn-primary btn-lg w-full" onClick={next} disabled={!val.trim()}>
            {step === 2 ? t("save") : t("next")}
          </button>
        </div>
      </div>
    </div>
        ...
        <div className="px-4 pb-4 space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {STATUS_KEYS.map((key) => (
              <button
                key={key}
                className={`btn btn-xs ${status === key ? "btn-primary" : "btn-outline"}`}
...
function SmartTab({ farmer, onRefresh }) {
  ...
  const handleSetStatus = (facilityId, status) => {
    setStatus(farmer.id, facilityId, status)
    onRefresh()
  }
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {FACILITIES.map((f) => (
          <FacilityExpand
            key={f.id}
            facility={f}
            status={getStatus(farmer.id, f.id)}
      ...
      <CollapsibleSection
        ...
        icon={<div className="w-8 h-8 rounded-full bg-warning/15 text-warning-content flex items-center justify-center"><svg viewBox="0 0 24 24" className="w-4 h-4" fill="currentColor"><path d="M16 6l2.29 2.29-4.88 4.88-4-4L2 16.59 3.41 18l6-6 4 4 6.3-6.29L22 12V6h-6Z" /></svg></div>}
      >
      ...
      <CollapsibleSection
        title={t("directSale")}
        icon={<div className="w-8 h-8 rounded-full bg-primary/15 text-primary flex items-center justify-center"><svg viewBox="0 0 24 24" className="w-4 h-4" fill="currentColor"><path d="M21 18v1a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v1" /><path d="M15 12h-5M12 9l3 3-3 3" /></svg></div>}
      >
      ...
      <CollapsibleSection
        title={t("envSoil")}
        icon={<div className="w-8 h-8 rounded-full bg-success/15 text-success flex items-center justify-center"><svg viewBox="0 0 24 24" className="w-4 h-4" fill="currentColor"><path d="M12 22a9 9 0 0 0 8.4-6H3.6A9 9 0 0 0 12 22Zm0-18C7.58 4 4 7.58 4 12c0 .34.02.68.07 1h15.86c.05-.32.07-.66.07-1 0-4.42-3.58-8-8-8Z" /></svg></div>}
      >
      ...
      <CollapsibleSection
        ...
        icon={<div className="w-8 h-8 rounded-full bg-warning/15 text-warning-content flex items-center justify-center"><svg viewBox="0 0 24 24" className="w-4 h-4" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2Zm1 15h-2v-2h2v2Zm0-4h-2V7h2v6Z" /></svg></div>}
      >
...
function FarmerHome({ farmer, onReset }) {
  const { t } = useTranslation()
  const [tab, setTab] = useState("home")
  const claimedCount = FACILITIES.filter((f) => getStatus(farmer.id, f.id) === "claimed").length
  const unclaimedFacilities = FACILITIES.filter((f) => {
    const s = getStatus(farmer.id, f.id)
    return s === "not-checked" || s === "available"
  })
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 pb-24">
      ...
      <div className="card bg-gradient-to-r from-primary/10 to-secondary/10 border border-primary/20 p-5 mb-6">
        <div className="flex items-center gap-4">
          <div className="avatar placeholder">
            <div className="bg-primary text-primary-content rounded-full w-14 h-14 font-display text-2xl font-bold">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="card bg-base-100 border border-base-content/10 shadow-sm">
              <div className="card-body p-5">
                <AudioText text={t("cropCalculator")} as="h3" className="font-display font-bold text-neutral" />
                <p className="text-sm text-base-content/70">{t("sellDirectDesc")}</p>
              </div>
            </div>
            <div className="card bg-base-100 border border-base-content/10 shadow-sm">
              <div className="card-body p-5">
          ...
          <div className="card bg-base-100 border border-base-content/10">
            <div className="card-body p-5">
              ...
              <div className="grid sm:grid-cols-3 gap-4 text-sm text-base-content/70">
                <div>
                  <AudioText text={t("checkBenefits")} as="p" className="font-semibold text-neutral mb-1" />
                  <p>{t("checkBenefitsDesc")}</p>
                </div>
                <div>
                  <AudioText text={t("smartTools")} as="p" className="font-semibold text-neutral mb-1" />
                  <p>{t("smartToolsDesc")}</p>
                </div>
                <div>
                  <AudioText text={t("sellDirect")} as="p" className="font-semibold text-neutral mb-1" />
                  <p>{t("sellDirectDesc")}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {tab === "smart" && (
        <SmartTab farmer={farmer} onRefresh={() => setFarmer(getCurrentFarmer())} />
      )}
...
export default function Home() {
  const [farmer, setFarmer] = useState(getCurrentFarmer)

  if (!farmer) {
    return <SetupForm onDone={setFarmer} />
  }

  const handleReset = () => {
    localStorage.removeItem("kh_current_farmer")
    setFarmer(null)
  }

  return <FarmerHome farmer={farmer} onReset={handleReset} />
}
